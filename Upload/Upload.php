...
class Upload extends AbstractServiceSetter
{
    ...
    public function upload(UploadedFile $file, PropertyMapping $propertyMapping, $filter_name = NULL, $entity = NULL)
    {
        if ($filter_name === NULL) {
        ...
        if ($uploaded) {
            // apply filter
            return $this->doApplyFilter($file_name, $propertyMapping, $entity);
        }
    ...
    public function uploadByUrl($url, $entity, $file_property_name, $set_file = false)
    {
        ...
        // apply filter
        if ($this->doApplyFilter($newFileName, $propertyMapping, $entity)) {
    ...
    public function doApplyFilter($file_name, PropertyMapping $mapping, $entity = NULL)
    {
        // init
        ...
        // write
        $filePath = $provider->getRelativeDir() . '/' . $file_name;
        $filesystem->write($filePath, $response->getContent());
        
        // dispatch event
        if ($entity !== NULL) {
            $uploadEvent = new UploadEvent($mapping, $entity, $filePath);
